...
\begin{frame}{Ecuaciones de Navier-Stokes (II)}
  ...
  \only<3>{{\color{pink}$\bullet$} Ecuación de incompresibilidad }
  \only<4->{
  \emph{Datos}:
  \begin{itemize}
  \item<4-> $\nu>0$: coeficiente de viscosidad (cinemática)
    \par\hfill ``inverso del número de \textit{Reynolds}'' ($Re \sim 1/\nu$, si $U,L\sim 1$).
      \begin{itemize}
      \item Caso $Re \gg$ (``$\nu\ll$''): flujo laminar
      \item Caso $Re \ll$ (``$\nu\gg$''): flujo turbulento
      \end{itemize}
...
\item \emph{Navier-Stokes, expresión extendida}:
  ...
  \nota[pink]{6}{Divergencia nula \scriptsize (ecuación de incompresibilidad o de conservación de la masa)}
